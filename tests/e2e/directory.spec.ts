...
test("directory renders ten labeled demo profiles", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 2, name: "10 perfiles disponibles" })).toBeVisible();
...
test("directory combines a region filter and clears it", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 2, name: "1 perfil disponible" })).toBeVisible();
