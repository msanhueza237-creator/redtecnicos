              ...
              {result.data.map((review) => {
                ...
                return (
                  <tr key={review.id}>
                    ...
                    <td><strong>{review.customerName}</strong><small>{review.customerEmail}</small><small>{review.service} · Solicitud {review.requestStatus}</small></td>
  ...
  return (
    <section className="admin-page">
      ...
      <AdminTableCard title="Recibidas en esta ejecución" description={`${capturedReviews.length} evaluaciones vinculadas a solicitudes demo`}>
        {capturedReviews.length ? <table className="admin-table"><thead><tr><th>Cliente</th><th>Profesional</th><th>Nota</th><th>Comentario</th><th>Estado</th><th>Acción demo</th></tr></thead><tbody>{capturedReviews.map((review) => <tr key={review.id}><td><strong>{review.customerDisplayName}</strong><small>{review.customerEmail}</small></td><td>{review.professionalDisplayName}</td><td><span className="rating"><Star aria-hidden="true" fill="currentColor" size={14} /> {review.rating}/5</span></td><td>{review.comment}</td><td><AdminStatus tone="info">Pendiente</AdminStatus></td><td><DemoAction label="Moderar" /></td></tr>)}</tbody></table> : <div className="admin-empty-state"><strong>Aún no hay evaluaciones capturadas</strong><p>Completa el flujo demo de solicitud y calificación para verla aquí.</p></div>}
      </AdminTableCard>
