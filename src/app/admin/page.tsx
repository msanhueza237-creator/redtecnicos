...
import { listAdminQualificationDocuments } from "@/lib/admin/qualifications";
...
export default async function AdminPage() {
  if (isSupabaseAuthMode()) {
    const [result, reviewsResult, complaintsResult, galleryResult, documentsResult] = await Promise.all([
      listProfessionalApplications(10),
      listAdminReviews(100),
      listAdminComplaints(100),
      listAdminGalleryItems(100),
      listAdminQualificationDocuments(100),
    ]);
    ...
    const openComplaints = complaintsResult.data.filter((item) => !["resolved", "dismissed"].includes(item.status)).length;
    ...
    return (
      <section className="admin-page">
        ...
        {result.error || reviewsResult.error || complaintsResult.error || galleryResult.error || documentsResult.error ? <p className="auth-message" role="alert">{result.error ?? reviewsResult.error ?? complaintsResult.error ?? galleryResult.error ?? documentsResult.error}</p> : null}
        <div className="metric-grid admin-metrics">
          ...
          <article><span className="metric-icon"><FileSearch aria-hidden="true" size={20} /></span><span>Documentos por revisar</span><strong>{documentsResult.data.filter((item) => item.status === "pending_review").length}</strong><small><Link href="/admin/documentos">Abrir bandeja</Link></small></article>
